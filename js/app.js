...
let missed = 0;
var phrases = ["Stay hungry Stay foolish",
...
function addPhraseToDisplay(arr) {
var phraseSplit = arr.split('');
  for (var i=0;i<phraseSplit.length;i++){
    var li = document.createElement("LI");
    var character = document.createTextNode(i);
    li.appendChild(character);
    document.getElementById("phrase").appendChild(li);

    if(i === " "){li.class = ""}
    else{li.class = "letter"}
  }
};


document.getElementsByClassName("btn__reset")[0].addEventListener("click", startGame);
